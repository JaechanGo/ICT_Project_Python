...
wb = Workbook()
# ws = wb.active 새로운시트 추가이기에 필요없음
ws = wb.create_sheet("chart",0) #chart라는 새로운 시트를 처음위치에 추가
...
data = Reference(ws, min_col=2, max_col=4,min_row=2,max_row=5)#차트 만들 때 사용될 데이터 범위 지정
cate = Reference(ws, min_col=2,min_row=2,max_row=5)#데이터 카테고리 범위지정

barchart.add_data(data, titles_from_data=True)
barchart.set_categories(cate)
barchart.shape=1

ws.add_chart(barchart, 'F1') # 'F1'은 차트가 만들어질 위
wb.save('openpyxl_chart.xlsx')
